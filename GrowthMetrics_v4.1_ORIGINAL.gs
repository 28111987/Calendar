// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  P2P GROWTH METRICS DASHBOARD — Premium Analytics Engine v4.1             ║
// ║  Modern Dark-Mode UI  ·  Real-Time  ·  Auto-Refresh                       ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
//
// v4.1 FIX LOG:
//   • CRITICAL: All numeric values now explicitly converted to strings via num()
//     to prevent Google Sheets auto-formatting (%, $, dates)
//   • CRITICAL: Entire sheet set to plain text format ("@") before writing data
//   • Layout: Names left-aligned, currency right-aligned, counts/ranks centred
//   • Layout: Refined column widths for better proportionality
//   • Layout: Consistent vertical alignment across all cell types
//   • Layout: Section titles full-span (no accent pip eating into columns)
//   • Tables: Proper number format — counts show as "80" not "$80" or "8000.00%"
// ═══════════════════════════════════════════════════════════════════════════════
// ─── CONFIGURATION ───────────────────────────────────────────────────────────
...
var P = {
  ...
  bg0       : "#0d1117",   // deepest background (canvas)
  bg1       : "#161b22",   // card / section background
  bg2       : "#1c2333",   // elevated surface
  bg3       : "#21293a",   // hover / subtle lift
  // Text
  tx0       : "#ffffff",   // primary text (headings, KPI values)
  tx1       : "#e6edf3",   // secondary text (body)
  tx2       : "#8b949e",   // muted text (labels, captions)
  tx3       : "#6e7681",   // disabled / footnotes
  // Accents
  gold      : "#f0b429",
  azure     : "#58a6ff",
  emerald   : "#3fb950",
...
var L = {
  ...
  colW: [
    34,    // A  – left gutter
    88,    // B  – Rank / Month / Type
    195,   // C  – primary name
    142,   // D  – metric 1
    142,   // E  – metric 2
    130,   // F  – metric 3
    88,    // G  – centre column
    88,    // H  – Rank / Month / Type
    195,   // I  – primary name
    148,   // J  – metric 1
    140,   // K  – metric 2
    130,   // L  – metric 3
    165,   // M  – metric 4 / visual
    34     // N  – right gutter
  ],
  // Row heights
  hTitle     : 70,
  ...
  for (var m = 0; m < MONTH_TABS.length; m++) {
    ...
    for (var r = 0; r < vals.length; r++) {
      ...
      allRows.push({
        ...
        status: sts, cost: cst, clientType: ctp,
...
// ─── Value Formatters ────────────────────────────────────────────────────────
// CRITICAL: Every value written to a cell MUST be a string.
// Google Sheets auto-formats raw numbers as %, $, or dates.
function clean(v)  { return String(v || "").trim(); }
function isDrop(v) { return v.toLowerCase().indexOf("select") !== -1; }
/** Format number as integer string:  80 → "80" */
function num(n)    { return String(Math.round(n)); }
/** Format as currency string:  1234 → "$1,234" */
function cur(n)    { return "$" + Math.round(n).toLocaleString("en-US"); }
/** Format as percentage string:  12.345 → "12.3%" */
function pct(n)    { return n.toFixed(1) + "%"; }
/** Format as decimal string:  3.14159 → "3.1" */
...
function analyse(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (!cM[d.domain]) cM[d.domain] = {p:0, r:0, v:0, rq:{}};
    cM[d.domain].p++;
    if (d.isConverted) { cM[d.domain].v++; cM[d.domain].r += d.cost; }
    if (d.requirement) cM[d.domain].rq[d.requirement] = true;
  }
  var cA = [];
  for (var k in cM) cA.push({n:k, p:cM[k].p, r:cM[k].r, v:cM[k].v, cr:dv(cM[k].v, cM[k].p)*100, rq:Object.keys(cM[k].rq).join(", ")});
...
function initSheet(sh) {
  ...
  // CRITICAL: Force plain-text format on entire sheet to prevent auto-formatting
...
function secTitle(sh, row, cS, cN, text, accent) {
  ...
  // Left accent pip (1 col)
  sh.getRange(row, cS, 1, 1).setBackground(accent);
  // Title text spans remaining columns
...
/** Native SPARKLINE progress bar — renders as a real smooth bar graph in the cell */
function sparkBar(sh, row, col, value, max, colour) {
  if (max <= 0) return;
  var pct = Math.min(value / max, 1);
  var bg  = P.bg3;  // unfilled portion colour
  var cell = sh.getRange(row, col);
  cell.setNumberFormat("");  // reset from "@" (plain text) so formula works
  cell.setFormula(
...
/** Style a mini-bar cell */
function barStyle(sh, row, col, colour) {
  sh.getRange(row, col).setFontColor(colour || P.gold).setFontFamily("Courier New").setFontSize(9);
}
// ═══════════════════════════════════════════════════════════════════════════════
// KPI CARD RENDERER
// ═══════════════════════════════════════════════════════════════════════════════
...
function updateGrowthMetrics() {
  ...
  sh.clear();
  sh.clearFormats();
  sh.setFrozenRows(0);   // explicitly unfreeze any rows from prior runs
  ...
  row++;
  spacer(sh, row); row++;
  // Clear ALL borders from entire header area (removes green border artifacts)
  ...
  for (var pi = 0; pi < pItems.length; pi++) {
    ...
    sparkBar(sh, row, 5, pItems[pi].v, A.tQ, pItems[pi].clr);
    row++;
  }
  ...
  for (var ni = 0; ni < nKeys.length; ni++) {
    ...
    tRow(sh, nRow, 8, [
      nKeys[ni],
      num(n.c),
      num(n.v),                              // ← Fixed: was showing "$19" etc.
      cur(n.r),
      pct(dv(n.v, n.c) * 100),
      pct(dv(n.r, tNR) * 100)
    ], ni, "bold");
  ...
  spacer(sh, row); row++;
  // ┌─────────────────────────────────────────────────────────────────────────┐
  // │  SERVICE FREQUENCY RANKING — Full Width                                │
  // └─────────────────────────────────────────────────────────────────────────┘
  secTitle(sh, row, 2, 12, "SERVICE FREQUENCY RANKING", P.azure);
  row++;
  ...
  for (var i = 0; i < A.sA.length; i++) {
    ...
    tRow(sh, row, 2, [
      num(i + 1),
      A.sA[i].n,           // service name
      num(A.sA[i].c),      // volume
      num(A.sA[i].v),      // converted
      pct(A.sA[i].cr),     // conv %
      cur(A.sA[i].r),      // revenue
      svcRevShare,          // rev share
      svcAvgRev,            // avg rev/conv
      "",                   // demand (sparkline)
      "", "", ""
    ], i);
    leftAlign(sh, row, 3);   // service name left
    sparkBar(sh, row, 10, A.sA[i].c, maxSvcCount, P.azure);
    row++;
  }
  // Service totals
  ...
  row++;
  gBorder(sh, svcStart - 1, 2, row - svcStart + 1, 12);
  spacer(sh, row); row++;
  accentDiv(sh, row, P.borderDk); row++;
  spacer(sh, row); row++;
  // ┌─────────────────────────────────────────────────────────────────────────┐
  // │  TOP 10 REQUIREMENTS — Most Opted Creative Requirements                │
  // └─────────────────────────────────────────────────────────────────────────┘
  secTitle(sh, row, 2, 12, "TOP 10 REQUIREMENTS — Most Opted Creative Requirements", P.gold);
  row++;
  ...
  for (var i = 0; i < A.rA.length; i++) {
    ...
    tRow(sh, row, 2, [
      num(i + 1),
      A.rA[i].n,          // requirement name
      A.rA[i].s,          // service name
      num(A.rA[i].c),     // volume
      num(A.rA[i].v),     // converted
      cur(A.rA[i].r),     // revenue
      pct(A.rA[i].cr),    // conv %
      revShare,            // rev share
      avgRvConv,           // avg rev/conv
      "",                  // demand (sparkline)
      "", ""
    ], i);
    leftAlign(sh, row, 3);   // requirement name left
    leftAlign(sh, row, 4);   // service name left
    sparkBar(sh, row, 11, A.rA[i].c, maxReqCount, P.azure);
    row++;
  }
  // Totals
  ...
  row++;
  gBorder(sh, rqStart - 1, 2, row - rqStart + 1, 12);
  spacer(sh, row); row++;
  accentDiv(sh, row, P.borderDk); row++;
  spacer(sh, row); row++;
  // ┌─────────────────────────────────────────────────────────────────────────┐
  ...
  // └─────────────────────────────────────────────────────────────────────────┘
  secTitle(sh, row, 2, 12, "MONTHLY REVENUE & PERFORMANCE TRENDS", P.emerald);
  row++;
  ...
  for (var i = 0; i < A.mA.length; i++) {
    ...
    tRow(sh, row, 2, [
      A.mA[i].m,           // month
      num(A.mA[i].q),      // SQLs
      num(A.mA[i].v),      // converted
      pct(A.mA[i].cr),     // conv %
      cur(A.mA[i].r),      // revenue
      cur(A.mA[i].avg),    // avg deal
      mom,                  // MoM growth
      cur(cumRev),          // cumulative
      "",                   // momentum (sparkline)
      "", "", ""
    ], i);
    sparkBar(sh, row, 10, A.mA[i].r, maxMoRev, P.emerald);
  ...
  row++;
  gBorder(sh, moStart - 1, 2, row - moStart + 1, 12);
  spacer(sh, row); row++;
  accentDiv(sh, row, P.borderDk); row++;
  spacer(sh, row); row++;
  // ┌─────────────────────────────────────────────────────────────────────────┐
  ...
  // └─────────────────────────────────────────────────────────────────────────┘
  secTitle(sh, row, 2, 12, "SERVICE BREAKDOWN BY MONTH", P.amber);
  row++;
  var sbH = ["Month"];
  ...
  spacer(sh, row); row++;
  // ┌─────────────────────────────────────────────────────────────────────────┐
  // │  TOP 10 CLIENTS — Complete Analysis                                    │
  // └─────────────────────────────────────────────────────────────────────────┘
  secTitle(sh, row, 2, 12, "TOP 10 CLIENTS — Revenue, Conversions & Requirements", P.rose);
  row++;
  tHead(sh, row, 2, ["#", "Client", "Revenue", "Projects", "Converted", "Conv %", "Rev Share", "Avg Rev/Conv", "Key Requirements", "Revenue Visual", "", ""]);
  row++;
  var cdStart    = row;
  var maxCliRev  = A.cA.length > 0 ? A.cA[0].r : 1;
  for (var i = 0; i < A.cA.length; i++) {
    var rStr     = A.cA[i].rq.length > 50 ? A.cA[i].rq.substring(0, 47) + "…" : A.cA[i].rq;
    var avgRC    = A.cA[i].v > 0 ? cur(A.cA[i].r / A.cA[i].v) : "$0";
    var cliShare = pct(dv(A.cA[i].r, A.tR) * 100);
    tRow(sh, row, 2, [
      num(i + 1),
      A.cA[i].n,            // client name
      cur(A.cA[i].r),       // revenue
      num(A.cA[i].p),       // projects
      num(A.cA[i].v),       // converted
      pct(A.cA[i].cr),      // conv %
      cliShare,              // revenue share
      avgRC,                 // avg rev/conv
      rStr,                  // key requirements
      "",                    // revenue visual (sparkline)
      "", ""
    ], i);
    leftAlign(sh, row, 3);   // client name left
    leftAlign(sh, row, 10);  // requirements left
    sh.getRange(row, 10).setFontSize(9);
    sparkBar(sh, row, 11, A.cA[i].r, maxCliRev, P.rose);
    row++;
  }
  // Client totals
  ...
  row++;
  gBorder(sh, cdStart - 1, 2, row - cdStart + 1, 12);
  spacer(sh, row); row++;
  // ┌─────────────────────────────────────────────────────────────────────────┐
  // │  FOOTER                                                                │
  // └─────────────────────────────────────────────────────────────────────────┘
